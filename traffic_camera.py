        ...
        while self._running:
            ...
            if cap is None:
                if not self._running:
                    break
                try:
                    # Use CAP_FFMPEG backend explicitly and reduce internal
                    # buffer to 1 frame.  This prevents the libavformat
                    # 'Invalid stream index' assertion that some IP Webcam
                    # Android versions trigger with the default AVFORMAT demuxer.
                    if isinstance(self.url, str):
                        new_cap = cv2.VideoCapture(self.url, cv2.CAP_FFMPEG)
                    else:
                        new_cap = cv2.VideoCapture(self.url)  # int index → default
